...
当前 MVP 的素材生成与导出流程仍可在前端本地演示。PR13 允许在本地配置百炼 API Key 后使用 Qwen 输出 `AssetPackPlan`；LLM 只作为 Planner / Art Director，不直接生成图片。本地 SVG Renderer 和 fallback 始终保留，无 API Key、LLM 关闭或调用失败都不影响核心演示。项目目前没有接入 LangChain、MCP、真实 Tool Calling、图像生成模型、数据库、登录、云部署或多 Agent 系统，也未提供批量 PNG 单独下载。
...
game-asset-forge/
  apps/
    ...
    api/                  # FastAPI 健康检查、fallback 与可选百炼 Art Planner API
  packages/
...
打开终端提示的本地地址，即可使用素材生成、预览、PNG、`metadata.json`、ZIP 与 Sprite Sheet 下载，这些核心功能均在浏览器本地完成。使用「AI 需求规划」获取 `AssetPackPlan` 时，需要同时启动后端；后端不可用时仍可手动配置表单。
...
启用后，`POST /api/agent/plan-pack` 使用百炼 OpenAI-compatible API 的 JSON Mode 生成并校验完整 `AssetPackPlan`。调用失败、JSON 无法解析或 Schema 校验失败时，接口仍返回 `success=true`、`source=fallback` 的本地计划。
...
3. 规划结果会显示来源与素材包摘要；手动调整表单后会回到本地规划。
...
9. 简述当前百炼输出结构化 `AssetPackPlan`，未来再扩展 Tool Calling、LangChain 与 MCP。
...
9. 未来 Tool Calling、LangChain 与 MCP 能力必须在当前本地 MVP 保持可运行的前提下独立推进。
