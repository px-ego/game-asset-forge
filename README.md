...
- Agent Pipeline Skeleton：前端以本地 deterministic pipeline 串联 Planner、Skill、Renderer 与 Validation，不调用真实模型。
- Variant 素材表现：同类型多份素材拥有不同名称、描述与可见装饰差异，例如裂纹金币、符文金币与宝石金币。

## 范围边界

当前 MVP 的核心素材生成与导出流程可以在前端本地演示。AI 需求规划面板调用的是后端 fallback 规则解析，不调用真实 LLM；未启动后端时，用户仍可手动选择参数并完成素材生成与导出。PR12 引入的是无 API Key 可运行的本地 Agent Pipeline Skeleton，不是模型 Agent。项目目前没有接入真实 LLM、LangChain、MCP、Stable Diffusion、数据库、用户登录、云部署或多 Agent 系统，也未提供批量 PNG 单独下载。
...
前端源码按职责分为 `api/`（planner 请求与响应校验）、`agent/`（本地 Planner/Skills/Validation/Pipeline）、`components/`（规划面板与预览组件）、`features/asset-generator/`（选项与保留的旧生成模块）、`exporters/`（导出工具）和 `types/`（共享数据结构）。
...
3. 点击“生成素材”后由本地 Agent Pipeline 生成带 variant 的 SVG 预览卡片；规划成功后不会自动生成。
...
13. Agent Pipeline：选择金币或史莱姆并将数量设为 `4`，确认卡片显示不同名称或 variant，且 SVG 外观可见不同。
14. 扩展 metadata：下载 `metadata.json`，确认各素材除既有字段外保留 `name`、`variant` 与 `description`。
...
8. 未来真实 LLM、Structured Output、Tool Calling、LangChain 与 MCP 能力必须在当前本地 MVP 保持可运行的前提下独立推进。
