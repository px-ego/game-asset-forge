...
当前 MVP 的核心流程已经可以在前端本地演示。PR10 新增的 Planner API 是本地 fallback 规则解析，不调用真实 LLM。项目目前没有接入 LLM、LangChain、MCP、Stable Diffusion、数据库、用户登录、云部署或多 Agent 系统，也未提供批量 PNG 单独下载。
...
前端源码按职责分为 `components/`（预览组件）、`features/asset-generator/`（规则生成与选项）、`exporters/`（导出工具）和 `types/`（共享数据结构），便于未来在不改变本地渲染链路的前提下接入 Planner。
...
打开终端提示的本地地址，即可使用 MVP 核心功能。素材生成、预览、PNG、`metadata.json`、ZIP 与 Sprite Sheet 下载均在浏览器本地完成，演示核心流程不要求后端启动。
...
6. 在单张卡片中点击“下载 PNG”，或在预览区域下载 `metadata.json`、ZIP 资源包与 Sprite Sheet。
...
9. 简述未来可加入 LLM Planner、Structured Output、Function Calling、LangChain 与 MCP；这些能力当前未实现。
...
6. 未来增强能力必须在当前本地 MVP 保持可运行的前提下独立推进。
