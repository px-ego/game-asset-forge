...
PR13 已接入可选的百炼 LLM Art Planner V2 与结构化 `AssetPackPlan` 校验。当前仍未接入 Function Calling / Tool Calling、LangChain、MCP 或图像生成模型。
...
1. Function Calling / Tool Calling：在稳定 `AssetPackPlan` 契约上调用校验、渲染、导出等工具。
...
上述能力均属于后续增强；当前完成范围为可选百炼 Art Planner 与始终可运行的本地渲染/fallback 链路。
