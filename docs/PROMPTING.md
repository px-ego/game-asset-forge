...
现有代码保留可选的百炼 LLM Art Director 与结构化规划校验：模型只返回轻量 `ArtDirectionPlan`，本地 Skill Layer 将其扩展为完整 `AssetPackPlan`。本轮新增后端 Tool Registry / Skill Registry，但当前仍未接入由模型驱动的 Function Calling / Tool Calling、LangChain、MCP 或图像生成模型。
...
当前素材生成仍由浏览器端本地规则完成。启用 `LLM_ENABLED=true` 且配置百炼 Key 时，`/api/agent/plan-pack` 让 Qwen 规划短小的艺术方向，后端本地装配完整素材包；未启用、无 Key、请求失败、JSON 解析失败或 Pydantic 校验失败时，后端自动返回本地 fallback 计划。后端未启动时，用户仍可手动选择参数并完成 SVG 预览与本地导出。
...
接口响应仍为结构化 `AssetPackPlan`。其内部 LLM 输出已缩小为 `ArtDirectionPlan`，例如：

```json
{"goal":"地牢像素套装","theme":"dungeon","style":"pixel","size":64,"count":4,"assetTypes":["coin","potion","slime"],"paletteIntent":{"primaryMood":"dark","accentMood":"crystal","keywords":["dark","crystal"]},"variantIntent":{"coin":["rune"],"potion":["crystal"],"slime":["shadow"]},"globalStyleHints":["高对比"]}
```

本地 assembler 再使用既有色板与 variant 规则生成前端接收的完整计划，包含色板、全局风格提示和素材项：
...
- system prompt 明确百炼是 Art Director，只允许输出 compact `ArtDirectionPlan`；禁止输出完整 assets、长 description、`renderHints` 或 seed，并限制所有数组最多 5 个短字符串。
- 调用使用百炼 OpenAI-compatible chat completion 与 JSON Mode：`response_format={"type": "json_object"}` 作为兼容请求扩展传入。
- 响应先尝试从 JSON 代码围栏或前后说明中提取完整对象，再经 `json.loads` 与 Pydantic `ArtDirectionPlan` 校验；本地 assembler 消费 `paletteIntent`、`variantIntent` 和 `globalStyleHints`，生成严格 `AssetPackPlan`。截断 JSON、非法枚举或最终计划不合法仍会安全 fallback。
...
4. 百炼调用使用 `timeout=60.0`、`max_retries=0`、`temperature=0.2` 与 `max_tokens=700`；模型不再重复输出每个 asset，因此 JSON 较短、被截断的风险更低。
5. `ArtDirectionPlan` 不包含 `seed` 或 `renderHints`；确定性 seed、variant 细节与 `glow: boolean` / `glowColor` 契约均由本地计划与渲染层负责。
...
1. 前端优先尝试消费可提供 `toolCalls` 的 `/api/agent/function-plan`；接口不可用时继续使用 `/api/agent/plan-pack` 与旧 fallback 路径。
2. 前端校验完整 `AssetPackPlan`，把基础选项写入表单，并缓存 external plan、来源及可用的 Agent Trace 信息。
3. 用户直接点击生成时，pipeline 消费 external plan；用户修改表单时，external plan 被清除并回到本地规划。
4. Renderer 消费 `palette` 与 `renderHints` 展示不同 variant；`glow` 控制是否发光，`glowColor` 优先提供高光颜色，否则沿用 `palette.accent`。导出继续使用本地 SVG/Canvas 路径。
5. `AgentTracePanel` 默认折叠展示 source、message、warnings、计划摘要及可用 `toolCalls`；它不参与渲染或导出。
6. 对于 `source=function_calling` 响应，工具执行发生在后端，前端只展示返回轨迹；当前尚未接入 LangChain 或 MCP。
...
前端 `AgentTrace` 保存最近一次规划或本地生成过程，包括 `source`、`message`、`warnings`、`toolCalls` 与可选 `AssetPackPlan` 摘要。`AgentToolCallTrace` 可以展示工具名称、执行参数、成功状态、消息和结果摘要；如果后端提供原始/归一化参数或耗时字段，面板也会安全展示。

手动表单生成无需 API Key：前端基于已完成的 deterministic pipeline 创建 `local-agent` trace，列出 `PaletteSkill`、`VariantSkill`、`RenderSkill` 与 `Validation`。后端 fallback 规划会产生 `fallback` trace；Function Calling 后端返回工具记录时，面板展示这些记录而不重复执行工具。
